...
export default function AgregarLibro() {
  ...
  const handleAgregarLibro = async (e) => {
    ...
    if (!titulo || !isbn || !autorId) {
      setError("Título, ISBN y autor son obligatorios");
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(data.error || "Error al agregar libro");
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="max-w-md p-4 bg-white/10 backdrop-blur-md rounded-xl shadow-md">
      <h2 className="text-xl font-bold mb-4 text-white">Agregar Libro</h2>
      {error && <p className="text-red-500 mb-2">{error}</p>}
      {success && <p className="text-green-500 mb-2">{success}</p>}
      <form className="flex flex-col gap-2" onSubmit={handleAgregarLibro}>
        <input value={titulo} onChange={(e) => setTitulo(e.target.value)} placeholder="Título" className="p-2 rounded border outline-none"/>
        <input value={isbn} onChange={(e) => setIsbn(e.target.value)} placeholder="ISBN" className="p-2 rounded border outline-none"/>
        ...
        <input value={editorialId} onChange={(e) => setEditorialId(e.target.value)} placeholder="ID editorial" className="p-2 rounded border outline-none"/>
        ...
        <button type="submit" className="bg-indigo-500 hover:bg-indigo-600 text-white p-2 rounded mt-2">Agregar Libro</button>
      </form>
    </div>
  );
}
